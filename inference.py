...
def predict_and_save(model, device, file_path, long_t, trans_t, long_shape, trans_shape, resize_target, out_dir, tta=False):
    model.eval()
    ...
    with torch.no_grad():
        if tta:
            ...
            # 2. Flip Horizontal
            xL_r_flip = torch.flip(xL_r, dims=[3])
            xT_r_flip = torch.flip(xT_r, dims=[3])
            segL_logits_flip, segT_logits_flip, cls_logits_flip = model(xL_r_flip, xT_r_flip)
            
            # Flip back
            segL_logits_flip = torch.flip(segL_logits_flip, dims=[3])
            segT_logits_flip = torch.flip(segT_logits_flip, dims=[3])
            
            # Average
            segL_logits = (segL_logits + segL_logits_flip) / 2.0
            segT_logits = (segT_logits + segT_logits_flip) / 2.0
...
def main():
    ...
    if args.tta:
        print("Test-Time Augmentation (TTA) Enabled: Horizontal Flip")

    for idx in range(len(ds)):
        p, long_t, trans_t, long_shape, trans_shape = ds[idx]
        out_path = predict_and_save(model, device, p, long_t, trans_t, long_shape, trans_shape, args.resize_target, out_dir, tta=args.tta)
